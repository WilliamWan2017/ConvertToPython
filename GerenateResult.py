...
def HA1(env, cstate=0):
    ...
    def Initial0(theta,x,y,phi,Initial0_FT,L2_FT,LocationEnd_FT,prev_time):
        curr_time=env.now
        vals={S.sympify('theta(t)'): theta,S.sympify('x(t)'): x,S.sympify('y(t)'): y,S.sympify('phi(t)'): phi}
        # the edge guard take preference
        if x>3:
            print('%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % (curr_time,theta,x,y,phi))  
            LocationEnd_FT=True   
            Initial0_FT=None                                       
            return 2, 0, theta,x,y,phi, Initial0_FT,L2_FT,LocationEnd_FT, curr_time
        if (((y>=0.8) and (x>=3.2)) or ((y<=-0.4) and (x<=2.8))):
            print('%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % (curr_time,theta,x,y,phi))  
            L2_FT=True   
            Initial0_FT=None                                       
            return 1, 0, theta,x,y,phi, Initial0_FT,L2_FT,LocationEnd_FT, curr_time
            ...
            if abs(x- 2.8) > Initial0_ode_x.vtol:
                dx= min( Initial0_ode_x.delta(vals, quanta=( 2.8- x),
                                      other_odes=[Initial0_ode_theta,Initial0_ode_y,Initial0_ode_phi]),dx )
            else:
                x=  2.8
                dx= 0
            if abs(y- 0.8) > Initial0_ode_y.vtol:
                dy= min( Initial0_ode_y.delta(vals, quanta=( 0.8- y),
                                      other_odes=[Initial0_ode_theta,Initial0_ode_x,Initial0_ode_phi]),dy )
            else:
                y=  0.8
                dy= 0
            ...
            return 0, Return_Delta, theta,x,y,phi, Initial0_FT,L2_FT,LocationEnd_FT, curr_time
        else:
    ...
    # Returning state, delta, value, loc1_FT, loc2_FT
    def L2(theta,x,y,phi,Initial0_FT,L2_FT,LocationEnd_FT,prev_time):
        curr_time=env.now
        vals={S.sympify('theta(t)'): theta,S.sympify('x(t)'): x,S.sympify('y(t)'): y,S.sympify('phi(t)'): phi}
        # the edge guard take preference
        if x>3:
        ...
        if not (((y>=0.8) and (x>=3.2)) or ((y<=-0.4) and (x<=2.8))):
            ...
            Initial0_FT=True   
            L2_FT=None                                       
            return 0, 0, theta,x,y,phi, Initial0_FT,L2_FT,LocationEnd_FT, curr_time
            ...
            return 1, Return_Delta, theta,x,y,phi, Initial0_FT,L2_FT,LocationEnd_FT, curr_time
        else:
            raise RuntimeError('Reached unreachable branch'
                               ' in L2')
        # Location End is end state in this example.
    def LocationEnd(theta,x,y,phi,Initial0_FT,L2_FT,LocationEnd_FT,prev_time):
        global step
  ...
  # The dictionary for the switch statement.
    ...
    prev_time = env.now
    while(True):
        (cstate, delta, theta,x,y,phi,Initial0_FT,L2_FT,LocationEnd_FT, prev_time) = switch_case[cstate](theta,x,y,phi,Initial0_FT,L2_FT,LocationEnd_FT,
                                                            prev_time)
