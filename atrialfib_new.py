...
def q(env, cstate=0):
    ...
    Loc0_ode_tau=ODE(env,lvalue= S.sympify('diff(tau(t))'),
                                                  rvalue=S.sympify('1.00000000000000'),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc0_ode_u=ODE(env,lvalue= S.sympify('diff(u(t))'),
                                                  rvalue=S.sympify(-jfi1 + stim - jsi1 - S.sympify('u(t)')/EPITO1),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc0_ode_w=ODE(env,lvalue= S.sympify('diff(w(t))'),
                                                  rvalue=S.sympify((-S.sympify('w(t)') + 1.0 - S.sympify('u(t)')/EPITWINF)/(EPITWM1 + (-EPITWM1 + EPITWM2)*(1.0/(S.exp(((-2)*EPIKWM)*(-EPIUS + S.sympify('u(t)'))) + 1)))),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc0_ode_v=ODE(env,lvalue= S.sympify('diff(v(t))'),
                                                  rvalue=S.sympify((-S.sympify('v(t)') + 1.0)/EPITVM1),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc0_ode_s=ODE(env,lvalue= S.sympify('diff(s(t))'),
                                                  rvalue=S.sympify((-S.sympify('s(t)') + 1/(S.exp(((-2)*EPIKS)*(-EPIUS + S.sympify('u(t)'))) + 1))/EPITS1),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc1_ode_tau=ODE(env,lvalue= S.sympify('diff(tau(t))'),
                                                  rvalue=S.sympify('1.00000000000000'),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc1_ode_u=ODE(env,lvalue= S.sympify('diff(u(t))'),
                                                  rvalue=S.sympify(-jfi3 + stim - 1.0/((-EPITSO1 + EPITSO2)*(S.exp(S.sympify('u(t)')*((-2)*EPIKSO)) + 1))),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc1_ode_w=ODE(env,lvalue= S.sympify('diff(w(t))'),
                                                  rvalue=S.sympify((-S.sympify('w(t)'))/EPITWP),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc1_ode_v=ODE(env,lvalue= S.sympify('diff(v(t))'),
                                                  rvalue=S.sympify((-S.sympify('v(t)'))/EPITVM2),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    Loc1_ode_s=ODE(env,lvalue= S.sympify('diff(s(t))'),
                                                  rvalue=S.sympify((-S.sympify('s(t)') + 1/(S.exp(((-2)*EPIKS)*(-EPIUS + S.sympify('u(t)'))) + 1))/EPITS2),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    ...
    Loc2_ode_v=ODE(env,lvalue= S.sympify('diff(v(t))'),
                                                  rvalue=S.sympify((-S.sympify('v(t)'))/EPITVP),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    ...
    loc3_ode_u=ODE(env,lvalue= S.sympify('diff(u(t))'),
                                                  rvalue=S.sympify(stim + S.sympify('v(t)')*(-EPITHV + S.sympify('u(t)'))*(EPIUU - S.sympify('u(t)'))/EPITFI - (-EPITSO1 + EPITSO2)/(S.exp(((-2)*EPIKSO)*(-EPIUSO + S.sympify('u(t)'))) + 1) - 1.0/EPITSO1 - S.sympify('s(t)')*w/EPITSI),ttol=10**-3,iterations=100,vtol=0, simplify_poly=True)
    ...
    def Loc0(tau,u,w,v,s,Loc0_FT,Loc1_FT,Loc2_FT,loc3_FT,prev_time):
        curr_time=env.now
        vals={S.sympify('tau(t)'): tau,S.sympify('u(t)'): u,S.sympify('w(t)'): w,S.sympify('v(t)'): v,S.sympify('s(t)'): s}
        # the edge guard take preference
        if u>=0.006:
            
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'Loc0-1',curr_time,tau,u,w,v,s))  
            Loc1_FT=True   
            Loc0_FT=None                                       
            ...
            #else:
            Loc0_FT = False
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'Loc0-2',curr_time,tau,u,w,v,s))
            #set a Maximum value for delta 
            du=9999999 
            if abs(u- 0.006) > Loc0_ode_u.vtol:
                du= min( Loc0_ode_u.delta(vals, quanta=( 0.006- u),
                                      other_odes=[Loc0_ode_tau,Loc0_ode_w,Loc0_ode_v,Loc0_ode_s]),du )
            else:
                u=  0.006
                du= 0
    ...
    def Loc1(tau,u,w,v,s,Loc0_FT,Loc1_FT,Loc2_FT,loc3_FT,prev_time):
        ...
        if u>=0.013:
            ...
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'Loc1-1',curr_time,tau,u,w,v,s))  
            Loc2_FT=True   
            Loc1_FT=None                                       
            ...
            #else:
            Loc1_FT = False
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'Loc1-2',curr_time,tau,u,w,v,s))
            #set a Maximum value for delta 
            du=9999999 
            if abs(u- 0.013) > Loc1_ode_u.vtol:
                du= min( Loc1_ode_u.delta(vals, quanta=( 0.013- u),
                                      other_odes=[Loc1_ode_tau,Loc1_ode_w,Loc1_ode_v,Loc1_ode_s]),du )
            else:
                u=  0.013
                du= 0
            Return_Delta=min(99999,du)
    ...
    def Loc2(tau,u,w,v,s,Loc0_FT,Loc1_FT,Loc2_FT,loc3_FT,prev_time):
        ...
        if u>=0.3:
            ...
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'Loc2-1',curr_time,tau,u,w,v,s))  
            loc3_FT=True   
            Loc2_FT=None                                       
            ...
            #else:
            Loc2_FT = False
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'Loc2-2',curr_time,tau,u,w,v,s))
            #set a Maximum value for delta 
            du=9999999 
            if abs(u- 0.3) > Loc2_ode_u.vtol:
                du= min( Loc2_ode_u.delta(vals, quanta=( 0.3- u),
                                      other_odes=[Loc2_ode_tau,Loc2_ode_w,Loc2_ode_v,Loc2_ode_s]),du )
            else:
                u=  0.3
                du= 0
            Return_Delta=min(99999,du)
    ...
    def loc3(tau,u,w,v,s,Loc0_FT,Loc1_FT,Loc2_FT,loc3_FT,prev_time):
        ...
        if u >= 2.0:
            ...
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'loc3-1',curr_time,tau,u,w,v,s))  
            loc3_FT=True   
            loc3_FT=None                                       
            return 3, 0, tau,u,w,v,s, Loc0_FT,Loc1_FT,Loc2_FT,loc3_FT, curr_time
        elif u<=2.0:
            ...
            #else:
            loc3_FT = False
            print('%s %7.4f:%7.4f:%7.4f:%7.4f:%7.4f:%7.4f' % ( 'loc3-2',curr_time,tau,u,w,v,s))
            #set a Maximum value for delta 
            du=9999999 
            if abs(u- 2.0) > loc3_ode_u.vtol:
                du= min( loc3_ode_u.delta(vals, quanta=( 2.0- u),
                                      other_odes=[loc3_ode_tau,loc3_ode_w,loc3_ode_v,loc3_ode_s]),du )
            else:
                u=  2.0
                du= 0
            Return_Delta=min(99999,du)
